...
class AdventureView(discord.ui.View):
    ...
    async def attack(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if random.random() < dodge_chance:
            desc += f"💨 **{self.monster['name']}**이(가) 분신술로 **공격을 회피(MISS)**했습니다!\n"
        else:
        ...
        # 3. 몬스터의 반격 (보스일 경우: 1~3타 연타 + 흡혈)
        cat_max_hp = self.cat_data.get("max_hp", 50)
        cat_current_hp = self.cat_data.get("hp", cat_max_hp)

        if self.monster.get("is_boss"):
            # 구미호 보스: 1~3번 연속 타격 패턴
            hits = random.randint(1, 3)
            total_boss_damage = 0
            for _ in range(hits):
                single_dmg = random.randint(self.monster["atk"] - 2, self.monster["atk"] + 2)
                total_boss_damage += max(1, single_dmg)

            cat_current_hp = max(0, cat_current_hp - total_boss_damage)
            self.cat_data["hp"] = cat_current_hp

            # 흡혈 효과: 준 데미지의 50% 체력 회복 (최대 체력 제한)
            heal_amount = int(total_boss_damage * 0.5)
            self.monster["hp"] = min(self.monster_max_hp, self.monster["hp"] + heal_amount)

            desc += f"⚡ **{self.monster['name']}**의 난무! **{hits}번 연속 공격**으로 **{total_boss_damage}**의 피해! (흡혈로 HP `+{heal_amount}` 회복)\n"
        else:
            # 일반 몬스터: 단발 공격
